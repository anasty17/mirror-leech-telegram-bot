...
def _watch(bot, message, isZip=False, isLeech=False, multi=0):
    ...
    listener = MirrorListener(
        bot, message, isZip, isLeech=isLeech, pswd=pswd, tag=tag
    )
        ...
        if formats is not None:
            ...
            for _format in formats_dict:
                if len(formats_dict[_format]) == 1:
                    ...
                    buttons.sbutton(
                        str(buttonName), f"qu {msg_id} {video_format}"
                    )
                else:
                    buttons.sbutton(
    ...
    if multi > 1:
        ...
        sleep(3)
        Thread(
            target=_watch, args=(bot, nextmsg, isZip, isLeech, multi)
        ).start()
...
def _qual_subbuttons(task_id, qual, msg):
    ...
    for index, br in enumerate(tbrs):
        ...
        if fps != "":
            video_format = (
                f"bv*[height={height}][fps={fps}][ext={ext}][tbr{tbr}]"
            )
        else:
...
quality_handler = CallbackQueryHandler(
    select_format, pattern="qu", run_async=True
)
