...
def leechSet(update, context):
    ...
    choose_msg = sendMarkup(msg, update.message, button)
    Thread(target=auto_delete_message, args=(update.message, choose_msg)).start()
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        sendMessage("Reply to a photo to save custom thumbnail.", update.message)
