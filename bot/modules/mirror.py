...
from re import match, split as resplit, search
from bot.helper.ext_utils.shortenurl import short_url
from bot.helper.telegram_helper.filters import CustomFilters
from bot.helper.telegram_helper.bot_commands import BotCommands
from bot.helper.telegram_helper.button_build import ButtonMaker
from os import path as ospath, walk, remove as osremove, listdir
...
from bot.helper.ext_utils.fs_utils import (
    split as fssplit, get_base_name, get_path_size, clean_download)
from bot.helper.telegram_helper.message_utils import (
    sendMarkup, sendMessage, delete_all_messages, update_all_messages)
from bot.helper.ext_utils.bot_utils import (
    is_url, is_magnet, is_mega_link, is_gdtot_link, is_gdrive_link,
    get_content_type, get_mega_link_type)
from bot import (
    LOGGER, QB_SEED, INDEX_URL, VIEW_LINK, DOWNLOAD_DIR, TG_SPLIT_SIZE,
    BUTTON_SIX_URL, BUTTON_FIVE_URL, BUTTON_FOUR_URL, BUTTON_SIX_NAME,
    BLOCK_MEGA_LINKS, BUTTON_FIVE_NAME, BUTTON_FOUR_NAME, BLOCK_MEGA_FOLDER,
    Interval, aria2, dispatcher, download_dict, download_dict_lock)
            ...
            try:
                ...
                if ospath.isdir(m_path):
                    for dirpath, subdir, files in walk(m_path, topdown=False):
                        for file_ in files:
                            if (
                                search(r"\.part0*1.rar$", file_)
                                or search(r"\.7z.0*1$", file_)
                                or (
                                    file_.endswith(".rar")
                                    and not search(r"\.part\d+.rar$", file_)
                                )
                                or file_.endswith(".zip")
                                or search(r"\.zip.0*1$", file_)
                            ):
                        ...
                        for file_ in files:
                            if (
                                file_.endswith(".rar")
                                or search(r"\.r\d+$", file_)
                                or search(r"\.7z.\d+$", file_)
                                or search(r"\.z\d+$", file_)
                                or search(r"\.zip.\d+$", file_)
                                or file_.endswith(".zip")
                            ):
...
def _mirror(
    bot, update, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None
):
        ...
        if not is_url(link) and not is_magnet(link) or len(link) == 0:
            ...
            if file is None:
                ...
                listener = MirrorListener(
                    bot, update, isZip, extract, isQbit, isLeech, pswd, tag
                )
        ...
        if content_type is None or match(
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    return sendMessage(
                        f"ERROR: link got HTTP response: {resp.status_code}",
                        bot,
                        update,
                    )
            except Exception as e:
    ...
    if is_gdrive_link(link):
        ...
        if link_type == "folder" and BLOCK_MEGA_FOLDER:
            ...
            Thread(
                ...
                args=(link, f"{DOWNLOAD_DIR}{listener.uid}/", listener),
            ).start()

    elif isQbit and (is_magnet(link) or ospath.exists(link)):
        Thread(
            target=add_qb_torrent,
            args=(link, f"{DOWNLOAD_DIR}{listener.uid}", listener, qbitsel),
        ).start()
