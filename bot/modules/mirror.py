...
class MirrorListener:
    ...
    def onUploadComplete(
        self, link: str, size, files, folders, typ, name: str
    ):
        if (
            not self.isPrivate
            and INCOMPLETE_TASK_NOTIFIER
...
def _mirror(
    ...
    if is_gdrive_link(link):
        if not isZip and not extract and not isLeech:
            ...
            Thread(
                target=add_gd_download, args=(link, listener, is_gdtot)
            ).start()
...
def qb_unzip_leech(update, context):
    _mirror(
        context.bot, update.message, extract=True, isQbit=True, isLeech=True
    )
