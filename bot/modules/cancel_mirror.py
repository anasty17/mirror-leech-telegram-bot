...
def cancel_mirror(update, context):
    ...
    if len(args) > 1:
        ...
        if not dl:
            sendMessage(f"GID: <code>{gid}</code> Not Found.", update.message)
            return
    ...
    if dl.status() == MirrorStatus.STATUS_ARCHIVING:
        sendMessage("Archival in Progress, You Can't Cancel It.", update.message)
    elif dl.status() == MirrorStatus.STATUS_EXTRACTING:
        sendMessage("Extract in Progress, You Can't Cancel It.", update.message)
    elif dl.status() == MirrorStatus.STATUS_SPLITTING:
        sendMessage("Split in Progress, You Can't Cancel It.", update.message)
    else:
...
def cancell_all_buttons(update, context):
    ...
    sendMarkup('Choose tasks to cancel.', update.message, button)
