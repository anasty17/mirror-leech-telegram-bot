...
def countNode(update, context):
    ...
    if is_gdrive_link(link):
        msg = sendMessage(f"Counting: <code>{link}</code>", update.message)
        gd = GoogleDriveHelper()
        result = gd.count(link)
        deleteMessage(msg)
        cc = f'\n\n<b>cc: </b>{tag}'
        sendMessage(result + cc, update.message)
    else:
        sendMessage('Send Gdrive link along with command or by replying to the link by command', update.message)
