...
def torser(update, context):
    ...
    if SEARCH_API_LINK is None and SEARCH_PLUGINS is None:
        ...
        button = InlineKeyboardMarkup(buttons.build_menu(2))
        sendMarkup(
            "Choose tool to search:", context.bot, update.message, button
        )
    elif SEARCH_API_LINK is not None and SEARCH_PLUGINS is None:
        button = _api_buttons(user_id, "apisearch")
        sendMarkup(
            "Choose site to search:", context.bot, update.message, button
        )
...
def _search(key, site, message, method):
    if method.startswith("api"):
        ...
        try:
            ...
            msg = (
                f"<b>Found {min(search_results['total'], TELEGRAPH_LIMIT)}</b>"
            )
            if method == "apitrend":
...
torserbut_handler = CallbackQueryHandler(
    torserbut, pattern="torser", run_async=True
)
