...
from bot import LOGGER, SEARCH_PLUGINS, SEARCH_API_LINK, dispatcher, get_client
...
def torser(update, context):
    ...
    if SEARCH_API_LINK is not None and SEARCH_PLUGINS is not None:
        ...
        return sendMessage(
            "No API link or search PLUGINS added for this function", context.bot, update
        )
...
def _getResult(search_results, key, message, tool):
    ...
    path = [
        telegraph.create_page(title="Mirror-leech-bot Torrent Search", content=content)[
            "path"
        ]
