...
class DbManager:
    ...
    async def update_deploy_config(self):
        ...
        await self.db.settings.deployConfig.replace_one(
            {"_id": TgManager.ID}, config_file, upsert=True
        )
    ...
    async def update_config(self, dict_):
        ...
        await self.db.settings.config.update_one(
            {"_id": TgManager.ID}, {"$set": dict_}, upsert=True
        )
    ...
    async def update_aria2(self, key, value):
        ...
        await self.db.settings.aria2c.update_one(
            {"_id": TgManager.ID}, {"$set": {key: value}}, upsert=True
        )
    ...
    async def save_qbit_settings(self):
        if self._return:
            return
        await self.db.settings.qbittorrent.update_one(
            {"_id": TgManager.ID}, {"$set": qbit_options}, upsert=True
        )
    ...
    async def update_private_file(self, path):
        ...
        if await aiopath.exists(path):
            ...
            await self.db.settings.files.update_one(
                {"_id": TgManager.ID}, {"$set": {db_path: pf_bin}}, upsert=True
            )
            if path == "config.py":
                await self.update_deploy_config()
        else:
            await self.db.settings.files.update_one(
                {"_id": TgManager.ID}, {"$unset": {db_path: ""}}, upsert=True
            )
    ...
    async def update_nzb_config(self):
        ...
        await self.db.settings.nzb.replace_one(
            {"_id": TgManager.ID}, {"SABnzbd__ini": nzb_conf}, upsert=True
        )
    ...
    async def rss_update_all(self):
        ...
        for user_id in list(rss_dict.keys()):
            await self.db.rss[TgManager.ID].replace_one(
    ...
    async def rss_delete(self, user_id):
        if self._return:
            return
        await self.db.rss[TgManager.ID].delete_one({"_id": user_id})

    async def add_incomplete_task(self, cid, link, tag):
        if self._return:
            return
        await self.db.tasks[TgManager.ID].insert_one(
    ...
    async def rm_complete_task(self, link):
        if self._return:
            return
        await self.db.tasks[TgManager.ID].delete_one({"_id": link})

    async def get_incomplete_tasks(self):
        notifier_dict = {}
        if self._return:
            return notifier_dict
        if await self.db.tasks[TgManager.ID].find_one():
            rows = self.db.tasks[TgManager.ID].find({})
            async for row in rows:
                if row["cid"] in list(notifier_dict.keys()):
                    ...
                    notifier_dict[row["cid"]] = {row["tag"]: [row["_id"]]}
        await self.db.tasks[TgManager.ID].drop()
        return notifier_dict
