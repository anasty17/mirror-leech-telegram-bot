...
class TldvDownloader:
    ...
    def estimated_total_size(self):
        if self.completed_segments > 0 and self.total_segments > 0:
            return int((self.processed_bytes / self.completed_segments) * self.total_segments)
        return 0
    ...
    async def _download_segment(self, client, url, index, temp_dir):
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    raise Exception(f"Failed to download segment {index} after 3 attempts: {e}")
                await asyncio.sleep(1)

    async def download(self):
        self.is_downloading = True
        self.start_time = time.time()
        match = re.search(r"https?://(?:app\.)?tldv\.io/(?:app/meetings|watch)/([a-zA-Z0-9]+)", self.listener.link)
        if not match:
        ...
        async with AsyncClient(follow_redirects=True, timeout=30.0) as client:
            ...
            try:
                ...
                if resp.status_code in [401, 403]:
                    await self.listener.on_download_error("Authentication required. Set TLDV_TOKEN config or pass valid cookies.")
                    return
                if resp.status_code != 200:
                    await self.listener.on_download_error(f"Failed to fetch metadata (HTTP {resp.status_code})")
                    return
                meta_data = resp.json()
            except Exception as e:
                await self.listener.on_download_error(f"Failed to fetch watch-page: {e}")
                return

            meeting_name = meta_data.get("meeting", {}).get("name", "Untitled Meeting")
            ...
            safe_name = re.sub(r'[\\/*?:"<>|]', "", meeting_name).replace(" ", "_")
            ...
            try:
                resp = await client.get(playlist_url, headers=headers, timeout=20.0)
                if resp.status_code != 200:
                    await self.listener.on_download_error(f"Failed to fetch playlist (HTTP {resp.status_code})")
                    return
                playlist_content = resp.text
            except Exception as e:
                await self.listener.on_download_error(f"Failed to fetch playlist: {e}")
                return
            ...
            try:
                ...
                for raw_line in lines:
                    ...
                    if offset is None or base_url is None:
                        raise ValueError("Segment line found before #TLDVCONF header")
                    deciphered = caesar_decipher(line, offset)
                    segment_urls.append(base_url + deciphered)
            except Exception as e:
                await self.listener.on_download_error(f"Failed to parse playlist: {e}")
                return

            self.total_segments = len(segment_urls)
            if self.total_segments == 0:
                await self.listener.on_download_error("No segments found in the playlist")
                return
            ...
            try:
                ...
                if not self.listener.is_cancelled:
                    await self.listener.on_download_error(f"Error downloading segments: {e}")
            ...
            try:
                ...
                await self.listener.on_download_error(f"Failed to merge segments: {e}")
                await rmtree(temp_dir, ignore_errors=True)
            ...
            cmd = ["ffmpeg", "-y", "-i", output_ts, "-c", "copy", "-movflags", "+faststart", final_mp4]
            _, res_err, code = await cmd_exec(cmd)
            if os.path.exists(output_ts):
                await aioremove(output_ts)
            if code != 0:
                await self.listener.on_download_error(f"Remuxing failed with code {code}. Stderr: {res_err}")
                return
...
async def add_tldv_download(listener, path, headers=None):
    ...
    if add_to_queue:
        LOGGER.info(f"Start Queued Download with TldvDownloader: {listener.name}")
    else:
