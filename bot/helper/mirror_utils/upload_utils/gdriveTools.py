...
class GoogleDriveHelper:
    ...
    def __upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        while response is None:
            ...
            try:
                ...
                if err.resp.get("content-type", "").startswith(
                    "application/json"
                ):
                    reason = (
    ...
    def __cloneFolder(self, name, local_path, folder_id, parent_id):
        ...
        for file in files:
            if file.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                not file.get("name").lower().endswith(tuple(EXTENTION_FILTER))
            ):
    ...
    def __drive_query(
        self, parent_id, fileName, stopDup, isRecursive, itemType
    ):
        try:
            if isRecursive:
                if stopDup:
    ...
    def __download_folder(self, folder_id, path, folder_name):
        ...
        for item in result:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                path + filename
            ) and not filename.lower().endswith(tuple(EXTENTION_FILTER)):
