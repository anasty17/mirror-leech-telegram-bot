...
class RcloneTransferHelper:
    ...
    async def upload(self, path, size):
        ...
        if await aiopath.isdir(path):
            mime_type = "Folder"
            folders, files = await count_files_and_folders(path, self._listener.excludeExtensions)
            rc_path += f"/{self._listener.name}" if rc_path else self._listener.name
        else:
            if path.lower().endswith(tuple(self._listener.excludeExtensions)):
    ...
    def _getUpdatedCommand(self, config_path, source, destination, method):
        ext = "*.{" + ",".join(self._listener.excludeExtensions) + "}"
        cmd = [
            "rclone",
            method,
            "--fast-list",
