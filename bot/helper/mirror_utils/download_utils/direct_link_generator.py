...
def yandex_disk(url: str) -> str:
    ...
    try:
        link = re_findall(r"\b(https?://(yadi.sk|disk.yandex.com)\S+)", url)[
            0
        ][0]
    except IndexError:
...
def uptobox(url: str) -> str:
    ...
    if UPTOBOX_TOKEN is None:
        ...
        try:
            ...
            file_link = (
                "https://uptobox.com/api/link?token=%s&file_code=%s"
...
def racaty(url: str) -> str:
    ...
    dl_url = rsoup.find("a", {"id": "uniqueExpirylink"})["href"].replace(
        " ", "%20"
    )
    return dl_url
