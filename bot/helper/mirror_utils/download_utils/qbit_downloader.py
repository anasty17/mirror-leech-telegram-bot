...
class QbDownloader:
    ...
    def add_qb_torrent(self, link, path, select):
        ...
        try:
            ...
            LOGGER.info(
                f"QbitDownload started: {self.__name} - Hash: {self.ext_hash}"
            )
            self.periodic = setInterval(
                self.POLLING_INTERVAL, self.__qb_listener
            )
            if BASE_URL is not None and select:
                ...
                buttons.sbutton(
                    "Done Selecting", f"qbs done {gid} {self.ext_hash}"
                )
        ...
        try:
            ...
            if tor_info.state == "metaDL":
                ...
                if (
                    ...
                    with download_dict_lock:
                        if self.__listener.uid not in list(
                            download_dict.keys()
                        ):
