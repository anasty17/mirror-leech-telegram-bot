...
def start(update, context):
    buttons = ButtonMaker()
    buttons.buildbutton("Channel", "https://t.me/whydeadport")
    buttons.buildbutton("Owner", "https://t.me/Apex_Not_Legend")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f'''
Hello Sir I am Your Personal Mirror Assistant!
Type /{BotCommands.HelpCommand} to get a list of available commands
And Sorry This Service Only For Ahmed Hridoy'''
        sendMarkup(start_string, context.bot, update.message, reply_markup)
    else:
        sendMarkup('Not an Authorized user, Sorry Sir You Cannot Use My Service', context.bot, update.message, reply_markup)

def restart(update, context):
    restart_message = sendMessage("System Rebooting...", context.bot, update.message)
    if Interval:
        Interval[0].cancel()
        Interval.clear()
    alive.kill()
                ...
                for tag, links in data.items():
                     ...
                     for index, link in enumerate(links, start=1):
                         ...
                         if len(msg.encode()) > 4000:
                             if 'System Restarted Successfully!' in msg and cid == chat_id:
