...
def stats(update, context):
    ...
    sendMessage(stats, update.message)
...
'''
        ...
        sendMarkup('Not Authorized user, deploy your own mirror-leech bot', update.message, reply_markup)

def restart(update, context):
    restart_message = sendMessage("Restarting...", update.message)
    if Interval:
        Interval[0].cancel()
    alive.kill()
...
def ping(update, context):
    ...
    reply = sendMessage("Starting Ping", update.message)
    end_time = int(round(time() * 1000))
    editMessage(f'{end_time - start_time} ms', reply)


def log(update, context):
    sendLogFile(update.message)
...
def bot_help(update, context):
    ...
    sendMarkup(help_string, update.message, reply_markup)
